...
export default function Dashboard() {
  ...
  const updateCameraStatus = async (id, status) => {
    try {
      ...
      const response = await axios.put(`https://api-app-staging.wobot.ai/app/v1/update/camera/status`, payload, {
        headers: {
          'Authorization': `Bearer 4ApVMIn5sTxeW7GQ5VWeWiy`
        }
      });
  ...
  return (
    <>
      <div className="w-full px-8">
        ...
        <CameraTableFilters cameraData={cameraData} selectedLocation={selectedLocation} setSelectedLocation={setSelectedLocation} selectedStatus={selectedStatus} setSelectedStatus={setSelectedStatus} />
        <CameraTable cameraData={currentItems} updateCameraStatus={updateCameraStatus} />
        <PaginationForTable
